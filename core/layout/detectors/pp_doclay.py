...
mirroring the reference example:  model.config.id2label[label]
"""
...
_STR_TO_DOCLABEL: dict[str, DocLabel] = {
...
class PPDocLayoutDetector(BaseLayoutDetector):
    ...
    def _load(self) -> None:
        ...
        cache_dir = (
            str(Path(self.config.cache_dir).expanduser())
            if not self.config.model_path else None
        )
        device = self._resolve_device()

        self._image_processor = AutoImageProcessor.from_pretrained(model_id, cache_dir=cache_dir)
        self._model = AutoModelForObjectDetection.from_pretrained(model_id, cache_dir=cache_dir)
        self._model.to(device)
        self._model.eval()

        # Mirror reference: model.config.id2label[label]
        self._id2label = {int(k): v for k, v in self._model.config.id2label.items()}
        logger.info("[PPDocLayout] %d labels: %s", len(self._id2label), self._id2label)

    def _predict(self, image: Image.Image) -> list[LayoutRegion]:
        return self._post.process(self._run_inference(image.convert("RGB")))
    ...
    def _run_inference(self, image: Image.Image) -> list[LayoutRegion]:
        ...
        for idx, (score, label_id, box, poly) in enumerate(
            ...
            label_str = self._id2label.get(label_id.item(), "abandon")
            doc_label = _STR_TO_DOCLABEL.get(label_str.lower(), DocLabel.ABANDON)
            ...
            poly_list = None
            if poly is not None:
                poly_list = [[int(round(p[0])), int(round(p[1]))] for p in poly.tolist()]

            regions.append(LayoutRegion(
                index=idx, label=doc_label, score=score_val,
                bbox=[x1, y1, x2, y2], poly=poly_list,
            ))
        return regions
